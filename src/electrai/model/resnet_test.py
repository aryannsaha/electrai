"""
ResUNet3D with sinusoidal time embedding.

Identical to ResUNet3D (resunet.py) except:
  - ResBlock3DTime injects a time embedding after the first activation
  - forward(x, t) takes an explicit timestep tensor
"""
...
class ResUNet3DTime(nn.Module):
    def __init__(
        ...
        self.in_conv = ResBlock3DTime(in_channels, n_channels, kernel_size, temb_dim, use_checkpoint)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: (B, in_channels, H, W, D)  — input charge density (coarse/low-res)
            t: (B,)                        — timestep in [0, 1]

        Returns:
            (B, out_channels, H, W, D) — predicted charge density, charge-conserving
        """
